#
# A tool for converting Pronto format hex codes to lircd.conf format. Version 1.1
#
# Version History:
...
class CodeSequence:           # Handles codesequences parsing and conversion
    ...
    def ProcessPreamble(self,sPreamble):
        if sPreamble[0]<>"0000":
            raise Exception("Formats other than starting with 0000 are not supported!")
    ...
    def CreatePulses(self,sItems):
        ...
        if len(self.dPulseWidths)<>2*(self.lOnceSequenceLength+self.lRepeatableSequenceLength):
            raise Exception("Number of actual codes does not match the header information!")
...
if len(sys.argv)<>2:
	print "Pronto codes converter to lircd.conf format (version 1.10)"
	print 
	print "Usage:   pronto2lirc.py inputfile"
	print
	print "Inputfile can be:"
	print "         1.An xml file outputted by CCFTools/CCFDecompiler."
